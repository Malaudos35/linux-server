...
help(){
    echo "option :"
    echo "-r pour acctiver la connexion en root"
    echo "-f <valeur> nombre de tentatives de connexions authorisé"
    echo "-p <port> le port de connexion ssh"
}

info "installation ssh ..."

# Analyser les options fournies par l'utilisateur
while getopts ":rf:p:-:" option; do
    case "${option}" in
        r)
            root=yes
            ;;
